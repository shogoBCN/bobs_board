                ...
                html.Div(
                    ...
                    children = [
                        html.Div(
                            className = "card_frame graph_frame",
                            children = [
                              dcc.Graph(id = "product_sales_graph")])]), 
...
@app.callback(
    Output("region_total_sold_graph", "figure"),
    [Input("region_dropdown", "value")])
def plot_all_sales(region):

    data = asr[asr["region"] == region].groupby(["variable"])["sales"].sum().sort_values(ascending = False)
...
@app.callback(
    Output("region_time_series", "figure"),
    [Input("region_dropdown", "value")])
def plot_all_sales(region):
...
def update_total_fb(region):
    ...
    df_product_stats = pd.DataFrame({"product": products, "mean_tenure": product_tenure})
...
# progression graph
@app.callback(
    Output('progression_graph', 'figure'),
    [Input('product_dropdown', 'value')])
